...
| 黄金窗口（真值健康度 0.6~0.8）漏检率 | 25.8%（v1.0 死区设计为 40.3%，v1.3 优化至 25.8%） |
| 黄金窗口 RUL 不可估占比 | 9.8%（v1.0 为 35.1%） |
| RUL 中位相对误差 / MAE | 88.1% / 295.7 h（中位绝对误差 40.3 h） |
...
*RUL 预测 vs 真值散点（主口径，可估样本 n=2423，对数坐标）：图内标注均为实测值——80% CI 实测覆盖率 33.0%、MAE 295.7h、中位绝对误差 40.3h、近失效窗口中位 15.3h；106 个评分提前跌破阈值输出 0.0h 的样本未绘制。早期凸性高估与晚期代理评分噪声如实呈现。*
...
**Q2：RUL 是怎么算的？**
对最近 144 条（= 一整天，覆盖温度日周期完整周期，避免日周期伪斜率污染斜率估计）健康评分做最小二乘线性外推至 35 分失效阈值，按 1 tick = 10 分钟换算为小时；80% 置信区间由拟合残差的标准外推预测方差构造：√(1+1/n+(k-x̄)²/Sxx)·σ ÷ |b| × TICK_HOURS（margin 先除以斜率绝对值再换算量纲；v1.3 统一了真值与预测的小时量纲）。评估真值口径为"真值健康度首次跌破 0.35 的剩余时长"，经唯一换算点 `truth_rul_hours()` 统一为小时。
...
**Q5：健康分怎么校准？会不会偷看真值？**
单通道恶化度（z>0.5σ 起算，按灵敏度分配满量程跨度：振动 42σ、温度/电流 20σ、双边 8σ）加权融合后，经 17 节点保序回归（isotonic 分段线性）校准表映射，使校准评分 ≈ 100×真值健康度。校准表与跨度常数在 4 条独立仿真轨迹（seed 2026/11/22/33，约 1.9 万样本）上离线标定，与评估种子 7 不相交（无泄漏）；**运行时推理不读取真值 health 字段**，等价于工业现场用失效数据离线标定评分模型。
...
## 十、Roadmap（真实规划，均为尚未实现）

RUL 真值量纲修正与评估工件重生成、看板/评估/预警喂入口径统一、最小 CI 接入已在 v1.3 完成；以下为剩余真实规划项：

- [ ] `Pipeline.reset()` 收敛到流水线线程执行（队列/标志位），或为 `engine.health_history` 与 TwinModel 加与 DataStore 同款锁
- [ ] AlertEngine 查询接口返回 deepcopy、内存告警/工单截断 `[-500:]`、告警升级时收敛旧工单
- [ ] 补校准表构建脚本 `scripts/fit_calibration.py` 入库，使 docs §6.3 标定过程可从仓库复现
- [ ] pytest 包装三个 selftest，把"回归对比"固化为测试断言
- [ ] `TwinUpdater.save` 改原子写 + `demo_follow` 对未登记 device_id 跳过并告警
- [ ] 包结构化（pyproject/`__init__.py`）消除 4 处 sys.path hack；3D 电机轴加键槽标记
