...
> RUL 口径说明：评估按每循环 1 条记录喂入（1 条 = 1 tick = 10 分钟），真值与预测/置信区间统一为小时；看板演示流水线每 6 个循环仅把最后 1 条喂给引擎，其在线 RUL 与评估口径存在约 3 倍系统性差异（已知待统一项）。早期退化段线性外推存在单点最高约 68 倍的高估，该基线适用于趋势参考与回归对比，不应作为精确检修时刻依据。
>
